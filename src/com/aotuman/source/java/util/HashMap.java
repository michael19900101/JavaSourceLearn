...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    static final int MIN_TREEIFY_CAPACITY = 64; // 树化的另外一个参数，当哈希表的所有元素个数超过64，才允许树化
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        // 延迟初始化逻辑，第一次调用putval()方法的时候才进行初始化hashmapd对象中最耗内存的table
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)
            ...
            // 3.表示当前桶位已经树化了
                ...
                for (int binCount = 0; ; ++binCount) {
                    // 4.1 迭代到最后一个元素了,也没有找到要插入的key一致的node
                    // 说明要加入当前链表的末尾
                    if ((e = p.next) == null) {
                        p.next = newNode(hash, key, value, null);
                        // 当前链表的长度达到树化的标准了，需要进行树化
